...
function ManageSocialMediaImages() {
  useEffect(() => {
    document.title = 'Manage Social Media Images | Admin Dashboard | Smart Original Brand Online Shop';
  }, []);
  ...
  const {data: instagramImages = [], refetch} = useQuery({
  ...
  const uploadtoDatabase = async (uploadUrl) => {
    ...
    if (!uploadUrl) return;
    try {
      const res = await axiosSecure.post('/layout/instagramImage', {image: uploadUrl})
      toast.success('Insta Image uploaded successfully');
      refetch()
      setFile(null)
      setPreview('')
      setIsLoading(false)
  ...
  const handleDeleteImages = async (id) => {
    try {
      await axiosSecure.delete(`/layout/instagramImage/${id}`)
    ...
    <div className="space-y-5">
      <div className="border-1 border-gray-200 rounded-md">
        <div className="px-4 border-b-1 gap-2 py-2 flex flex-col md:flex-row items-center justify-between">
          <h1>Select instagram Image</h1>
          <p className="text-xs text-gray-500">File Type PNG (MIN. 384x384)</p>
        </div>
      ...
      <div className="border-1 border-gray-200 rounded-md">
      <div className="px-4 border-b-1 gap-2 py-2 flex flex-col md:flex-row items-center justify-between">
        <h1>Uploaded instagram Images</h1>
      </div>
      <div className="p-4">
...
const SelectedImagePreview = ({ image, index, handleDeleteImages }) => {
  return (
    <div key={index} className="p-2 border-1 max-w-96 rounded-md">
      ...
      <Button size="sm" radius="sm" onClick={() => handleDeleteImages(image._id)} className="w-full" color="danger">Delete</Button>
    </div>
  );
};


export default ManageSocialMediaImages